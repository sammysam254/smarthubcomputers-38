    ...
    <div className="space-y-6">
      ...
      <div ref={receiptRef} className="receipt bg-white p-8 border rounded-lg shadow-lg max-w-4xl mx-auto" style={{ fontFamily: 'Arial, sans-serif' }}>
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">Receipt #{receiptData.orderId || 'N/A'}</p>
        ...
        {/* Items Table */}
        <table className="table w-full border-collapse mb-6">
          <thead>
            <tr>
              <th className="border border-gray-300 p-3 bg-gray-100">Item Name</th>
              <th className="border border-gray-300 p-3 bg-gray-100">Quantity</th>
              <th className="border border-gray-300 p-3 bg-gray-100">Unit Price (KSH)</th>
              <th className="border border-gray-300 p-3 bg-gray-100">Total (KSH)</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="border border-gray-300 p-3">{receiptData.itemName}</td>
              <td className="border border-gray-300 p-3">{receiptData.quantity}</td>
              <td className="border border-gray-300 p-3">{receiptData.unitPrice.toLocaleString()}</td>
              <td className="border border-gray-300 p-3">{calculateSubtotal().toLocaleString()}</td>
            </tr>
            <tr>
              <td colSpan={3} className="border border-gray-300 p-3 font-bold">Subtotal</td>
              <td className="border border-gray-300 p-3 font-bold">KSH {calculateSubtotal().toLocaleString()}</td>
            </tr>
            <tr>
              <td colSpan={3} className="border border-gray-300 p-3 font-bold">VAT (16%)</td>
              <td className="border border-gray-300 p-3 font-bold">KSH {calculateVAT().toLocaleString()}</td>
            </tr>
            <tr className="total-row bg-gray-100">
              <td colSpan={3} className="border border-gray-300 p-3 font-bold">TOTAL</td>
              <td className="border border-gray-300 p-3 font-bold">KSH {calculateTotal().toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
        ...
        <div className="section mb-8">
          ...
          <p><strong>MPesa Transaction Code:</strong> {receiptData.mpesaCode}</p>
          <p><strong>Payment Status:</strong> Confirmed</p>
        </div>

        {/* Signatures */}
        <div className="signature-section flex justify-between mt-12">
          <div className="signature-box text-center" style={{ width: '200px' }}>
            <div className="signature-line border-b border-black mb-2" style={{ height: '40px' }}></div>
            <p className="font-bold">Customer Signature</p>
            <p className="text-sm">{receiptData.customerName}</p>
          </div>
          
          <div className="signature-box text-center" style={{ width: '200px' }}>
            <div className="signature-line border-b border-black mb-2" style={{ height: '40px' }}></div>
            <p className="font-bold">Delivered By</p>
            <p className="text-sm">{receiptData.deliveredBy}</p>
          </div>
          
          <div className="signature-box text-center" style={{ width: '200px' }}>
            <div className="signature-line border-b border-black mb-2" style={{ height: '40px' }}></div>
            <p className="font-bold">Sales Signature</p>
            <p className="text-sm">{receiptData.salesPerson}</p>
          </div>
        </div>
